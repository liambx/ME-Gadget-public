...
#ifdef HUBBLE_TABLE
  printf("Calculating the Drift, GravKick and HydroKick Table from Hubble Table.\n");
  for(i = 0; i < DRIFT_TABLE_LENGTH; i++)
    {
      gsl_spline_eval_integ_e(MeHubbleSplineDRIFT, exp(logTimeBegin), exp(logTimeBegin + ((logTimeMax - logTimeBegin) / DRIFT_TABLE_LENGTH) * (i + 1)),  All.MeHubbleAcc, &result);
      DriftTable[i] = result;


      gsl_spline_eval_integ_e(MeHubbleSplineGRAVKICK, exp(logTimeBegin), exp(logTimeBegin + ((logTimeMax - logTimeBegin) / DRIFT_TABLE_LENGTH) * (i + 1)),  All.MeHubbleAcc, &result);
      GravKickTable[i] = result;


      gsl_spline_eval_integ_e(MeHubbleSplineHYDROKICK, exp(logTimeBegin), exp(logTimeBegin + ((logTimeMax - logTimeBegin) / DRIFT_TABLE_LENGTH) * (i + 1)),  All.MeHubbleAcc, &result);
      HydroKickTable[i] = result;
      
      //printf("Hubble Table says DriftTable[%d]=%f,GravKickTable[%d]=%f,HydroKickTable[%d]=%f\n",i,DriftTable[i],i,GravKickTable[i],i,HydroKickTable[i]); // for debug

    }
  printf("Done with Calculating the Drift, GravKick and HydroKick Table from Hubble Table.\n");
#else
  for(i = 0; i < DRIFT_TABLE_LENGTH; i++)
    {
      ...
      //printf("Originally says DriftTable[%d]=%f,GravKickTable[%d]=%f,HydroKickTable[%d]=%f\n",i,DriftTable[i],i,GravKickTable[i],i,HydroKickTable[i]); //for debug
