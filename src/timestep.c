...
#endif
#ifdef VARG_TABLE
  double varg = gsl_spline_eval(All.MeVARGSpline, All.Time, All.MeVARGAcc);
#endif
  t0 = second();
...
#ifdef HUBBLE_TABLE
	  //printf("All.Time=%f,hubble_a=%f",All.Time,hubble_a); //for debugging
	  hubble_a = All.Hubble * gsl_spline_eval(All.MeHubbleSpline, All.Time, All.MeHubbleAcc);
	  //printf("All.Time=%f,hubble_a=%f",All.Time,hubble_a); //for debugging
	  //getchar();
#endif
...
#ifdef DMMASS_TABLE
	  double dmmass=All.MassTable[1];
	  //printf("dmmass=%f,current_time=%f\n",dmmass,All.Time); //for debugging
	  dmmass = gsl_spline_eval(All.MeDMMassSpline, All.Time, All.MeDMMassAcc);
	  //printf("dmmass=%f\n",dmmass); //for debugging

	  if(All.MassTable[1] != 0)
	  All.MassTable[1] = dmmass; 
	  
	  for(i = 0; i < NumPart; i++)
	  {
		  if(P[i].Type == 1)   /* Only type 1, dark matter particle mass will be changed */
		  {
			  P[i].Mass = dmmass;
		  }
	  }
  ...
  for(i = 0; i < NumPart; i++)
      ...
      if(P[i].Ti_endstep == All.Ti_Current)
	{
	  ...
	  for(j = 0; j < 3; j++)
	    {
#ifdef VARG_TABLE			
	      dv[j] = P[i].GravAccel[j] * varg * dt_gravkick;
#else
...
#ifdef DRAG_TABLE
	  double drag=0.0;
	  //printf("drag=%f,current_time=%f\n",drag,All.Time); //for debugging
	  drag = gsl_spline_eval(All.MeDRAGSpline, All.Time, All.MeDRAGAcc);
	  //printf("drag=%f\n",drag); //for debugging
#endif
      for(i = 0; i < NumPart; i++)
	{
	  for(j = 0; j < 3; j++)	/* do the kick */
	    {
#ifdef VARG_TABLE
			P[i].Vel[j] += P[i].GravPM[j] * varg * dt_gravkick;
#else
		    P[i].Vel[j] += P[i].GravPM[j] * dt_gravkick;
#endif			
#ifdef DRAG_TABLE
	  if(P[i].Type == 1)   /* Only type 1, dark matter particle velocity will be changed */
		{
		  P[i].Vel[j] += (P[i].Vel[j] - P[i].GravPM[j] * dt_gravkick) * drag * dt_gravkick;
		}
#endif
		}
	  if(P[i].Type == 0)
